...
Module.register('MMM-Holiday2', {
  ...
  getDom: function () {
    ...
    if (!this.loaded) {
      wrapper.innerHTML = this.translate('Gathering Holidays...')
    ...
    for (var i = 0; i < this.holidays.length; i++) {
      ...
      var rallDate =
        this.getTwoDigitDateFormat(dt.getDate()) +
        '/' +
        this.getTwoDigitDateFormat(dt.getMonth() + 1) +
        '/' +
        dt.getFullYear()
      var DateDiff = {
      ...
      if (this.holidays.length > 0) {
        if (
          DateDiff.inDays(d1, d2) > -1 &&
          DateDiff.inDays(d1, d2) <= this.config.days
        ) {
          if (todayDate === allDate) {
            HolidayColumn.classList.add('bright', 'xsmall', 'today', 'row')
            HolidayColumn.innerHTML =
              '<font color=#ece96a>&nbsp;&nbsp;<b>' +
              this.getFormatDate(allDate, rallDate) +
              '</b> ~ ' +
              holiday.summary +
              ' ' +
        ...
        HolidayColumn.innerHTML = this.translate('No Upcoming Holidays')
